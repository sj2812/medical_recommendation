...
Requires Neo4j running. Set NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, NEO4J_DATABASE (default: qbankuser) if needed.
"""
import os
...
def build_qbank_graph(driver, qbank: pd.DataFrame, clear_first: bool = True):
    ...
    with driver.session(database=NEO4J_DATABASE) as session:
        ...
        for d in tqdm(difficulties, desc="Difficulties"):
            session.execute_write(_merge_difficulty, str(d).strip())
        for t in tqdm(terms, desc="Terms"):
            session.execute_write(_merge_term, t)
        for p in tqdm(parent_terms, desc="Parent terms"):
    ...
    with driver.session(database=NEO4J_DATABASE) as session:
        for _, row in tqdm(q_meta.iterrows(), total=len(q_meta), desc="Questions"):
    ...
    # (Difficulty already linked once per question in _merge_question.)
    with driver.session(database=NEO4J_DATABASE) as session:
        for _, row in tqdm(qbank.iterrows(), total=len(qbank), desc="Qbank tags (Category/Term links)"):
            qid = row["question_id"]
            cat = str(row["category"]).strip()
            term = str(row["term"]).strip()
            parent = row["parent_term"]
            session.execute_write(_link_question_category, qid, cat)
            session.execute_write(_link_question_term, qid, term)
            session.execute_write(_link_term_category, term, cat)
            if pd.notna(parent) and str(parent).strip():
                session.execute_write(_link_term_parent, term, str(parent).strip())
...
def build_user_train_edges(driver, train_df: pd.DataFrame):
    ...
    with driver.session(database=NEO4J_DATABASE) as session:
        for _, row in tqdm(train_df.iterrows(), total=len(train_df), desc="Train (User-ANSWERED->Question)"):
            uid = row["user_id"]
            qid = row["question_id"]
