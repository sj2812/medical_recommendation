...
def run_kg_policy_training(
    ...
    *,
    ...
    """
    # Reproducibility: same seed so Docker (CPU) and local (GPU) give comparable loss scale
    ...
    def listwise_reward_ce_stable(scores, rewards, mask, temperature=1.0, reward_temp=1.0, reward_clip=2.0):
        # Only compute over valid positions; padding (mask==0) would give target>0 and logp=-inf -> NaN/inf loss.
        r = rewards.clamp(min=-reward_clip, max=reward_clip).float()
        r_masked = r.masked_fill(mask == 0, float("-inf"))
        target = torch.softmax((r_masked / reward_temp), dim=1)
        target = target * mask
        target = target / (target.sum(dim=1, keepdim=True).clamp(min=1e-8))
        logp = torch.log_softmax((scores / temperature).float(), dim=1)
        logp_safe = torch.where(mask > 0, logp, torch.zeros_like(logp))
        return -(target * logp_safe).sum(dim=1).mean()
    ...
    for ep in range(1, policy_epochs + 1):
        ...
        for batch in tqdm(loader, desc=f"KG-policy {ep}/{policy_epochs}"):
            ...
            with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
                loss_policy = listwise_reward_ce_stable(scores, rewards_t, mask_t, temperature=temperature, reward_temp=1.0, reward_clip=2.0)
                loss = loss_policy
